...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let dataset_dir = "tspider-parser/0225/dataset";
    let error_path = "tspider-parser/0225/output/error.csv";

    // 确保输出目录存在
    std::fs::create_dir_all("tspider-parser/0225/output")?;
    ...
    let (total_stats, sql_error_list) = runtime.block_on(async {
        ...
        // 并发限制
        let concurrency_limit = 4; // 调整为合适的并发数
